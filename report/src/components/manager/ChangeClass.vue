    ...
    <content>
      <div class="content">
        <el-form ref="form" :rules="rules" :model="form" label-width="120px">
          <el-form-item label="班级原名称：" size="small" prop="oldname">
            <el-col :span="14">
              <el-input v-model="form.oldname" :disabled="true"></el-input>
            </el-col>
          </el-form-item>
          <el-form-item label="班级新名称：" size="small" prop="newname">
            <el-col :span="14">
...
export default {
  ...
  mounted() {
    this.form.oldname = this.$route.params.className;
    let that = this;
    this.request({
      methods: "get",
      url: `${config.url}/admin/get_class_information`,
    }).then((res) => {
      let temp = res.data.find((v) => v.name === that.$route.params.className);
      this.form.people = temp.people;
      this.form.learntime = temp.learntime;
      this.form.price = temp.price;
      this.form.maxsum = temp.maxsum;
    });
  },
  data() {
    return {
    ...
    onSubmit(form) {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          this.request({
            methods: "get",
            url: `${config.url}/admin/change_class_information`,
            params: {
